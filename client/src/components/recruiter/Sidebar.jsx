...
import { 
  ...
  BarChart3, Building, Bell, Settings, Rocket, LogOut, X, PlusCircle
} from 'lucide-react';
...
export default function Sidebar({ isOpen, onClose }) {
  const navItems = [
    ...
    { name: 'Applicants', path: '/recruiter/applicants', icon: Users, badge: '12' },
    ...
    { name: 'Notifications', path: '/recruiter/notifications', icon: Bell, badge: '5' },
      ...
      <aside className={`fixed top-0 bottom-0 left-0 z-50 w-64 bg-[#0a0a0a] border-r border-gray-800 flex flex-col justify-between transition-transform duration-300 ease-in-out lg:translate-x-0 ${
        ...
        <div className="p-4 border-t border-gray-800 bg-[#050505]">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-9 h-9 rounded-md bg-gray-800 border border-gray-700 flex items-center justify-center font-bold text-white text-sm">
              AC
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-white truncate">Acme Corp</p>
              <p className="text-xs text-gray-500 truncate">HR Dashboard</p>
            </div>
          </div>
          <Link to="/login" className="flex items-center gap-2 text-xs font-medium text-gray-400 hover:text-red-400 transition-colors w-full px-2 py-1 rounded">
